...
def standardcalculator():
    ...
    global total #Creating a global total
...
#Creating MS function
    def MS():
        ...
        if(stored):
           ...
           clear_all()
           bar.insert(0, "0") #creating error handling message


#Creating MC function
    def MC():
        global stored
        stored = bar.delete(0, END)
        del stored
...
#Creating equals '=' function
    def equals():
       num2 = bar.get()
       equation = part1 + str(num2)
       clear_all()
       try:
           formula = parser.expr(equation).compile()
           result = eval(formula)
           clear_all()
           bar.insert(0, result)
       except Exception:
           clear_all()
           bar.insert(0, "Error press MC")
...
#########################
    ...
    #C button
    button5= Button(calwindow, text = "C", command = clear_all_2, font=10) #Creating text on box and calling MC function
